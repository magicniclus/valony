...
const StepEng = () => {
  ...
  const handleTitle = (step: number) => {
    switch (step) {
      case 1:
        setTitle("For more information, it’s here ");
        setPlaceholder("Name");
        break;
      case 2:
        setTitle("For more information, it’s here ");
      ...
      case 3:
        setTitle("For more information, it’s here ");
        setPhoneNumber(["Country Code", "06 XX XX XX XX"]);
        break;
      case 4:
        setFinalTitle(true);
        break;
    ...
    <>
      ...
      <div className="font-outfit">
        <div>
          <div
            className={`${
              step < 3 && "backgroundThree"
            } bg-transparent relative  md:mr-48 md:min-w-[470px]`}
          >
            <div
              className={`transition duration-300 ease-in-out ${
                getFocus
                  ? "-translate-y-[calc(100%px)] "
                  : "lg:flex translate-y-[calc(100%px)] "
              }`}
            >
              <p
                className={`lg:hidden flex flex-col font-outfit text-text uppercase text-[14px] transition duration-300 ease-in-out ${
                  step < 4 ? " " : "hidden"
                } ${
                  getFocus
                    ? "-translate-y-[calc(100%px)] "
                    : "lg:flex translate-y-[calc(100%+5px)] "
                } " : ""} `}
              >
                {title}{" "}
                <span className="text-textClear text-[10px]">
                  {getFocus
                    ? step === 1
                      ? "Enter your name"
                      : step === 2
              ...
              <p
                className={`lg:text-[16px] text-[16px] flex-col font-outfit uppercase items-end absolute top-0 left-0 transition duration-300 ease-in-out lg:flex hidden ${
                  getFocus ? "lg:-translate-x-[calc(100%+15px)] " : ""
                } ${step < 4 ? " " : "hidden"}`}
              >
                {title}
                <span className="text-textClear text-[10px]">
            ...
            <div className="flex">
              {step < 3 ? (
                ...
                <div
                  className={`flex md:w-[450px] justify-between ${
                    !getFocus && "backgroundTelOneContainer"
                  }`}
                >
                  <select
                    ...
                    } bg-transparent w-[25%] relative pb-3 text-[14px]`}
                  >
                    <option value="" disabled>
                      {getFocus ? "Country Code" : ""}
                    </option>
                    {/* Mise à jour pour afficher correctement les options d'indicatif */}
                    {listIndicatif.map((indicatif, index) => (
                      <option key={index} value={indicatif}>
                        {indicatif}
                      </option> // Utilisez indicatif ou une structure appropriée pour `value`
                    ))}
                  </select>
                  <input
                    type={"text"}
                    value={valueDisplay}
                    onChange={handleInputChange}
                    className={`backgroundTelOne bg-transparent w-[67%] relative pb-3 placeholder:text-text text-[14px]  ${
                      getFocus && "backgroudTelOneBorder"
                    }`}
                ...
                <div className="w-full flex flex-col mb-5">
                  ...
                  <div className="w-full flex md:justify-end">
                    <button
                      type="button"
                      className="bg-or text-white text-[20px] font-outfit py-3 w-full max-w-[129px] mt-5 rounded-full"
                      disabled={isButtonDisabledTwo}
                      onClick={handleSubmite}
                      id={step === 4 ? "conversion" : ""}
                    >
                      Envoyer
                    </button>
                  </div>
